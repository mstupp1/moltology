...
export async function captureComposite(options: CaptureCompositeOptions): Promise<string> {
  ...
  try {
    ...
    const chromeCmd = `"${CHROME_PATH}" --headless=new --disable-gpu --hide-scrollbars --window-size=${dims.width},${dims.height} --force-device-scale-factor=${scale} --screenshot="${options.outputPath}" "${targetUrl}"`
