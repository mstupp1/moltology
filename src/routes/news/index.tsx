...
export const Route = createFileRoute('/news/')({
  ...
  head: () => ({
    meta: [
      { title: 'Moltology News Network | Centralized Dispatches & Telemetry' },
      { name: 'description', content: 'Official news dispatches, AI telemetry, autonomous swarm reports, and sacrosanct carcinization updates.' },
      { property: 'og:title', content: 'Moltology News Network | Centralized Dispatches & Telemetry' },
      { property: 'og:description', content: 'Live intelligence feed covering agentic AI, test-time compute, and exoskeletal ascension.' },
    ],
  }),
...
function NewsIndexPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070b0b] text-gray-200 font-mono relative select-none flex flex-col justify-between">
      ...
      {/* Live Breaking News Ticker HUD */}
      <div className="w-full bg-[#04090b] border-b border-cyan-900/50 py-2 px-4 flex items-center gap-3 relative z-20 text-xs shadow-inner">
        <div className="flex items-center gap-2 px-3 py-1 bg-red-950/90 border border-red-500/80 text-red-400 font-bold uppercase tracking-wider text-[11px] shrink-0 chamfer-corner shadow-hud-red">
        ...
        <div className="overflow-hidden whitespace-nowrap text-cyan-300/90 text-xs font-mono flex-1">
          <div className="inline-block animate-marquee tracking-wide">
            <span className="text-red-400 font-bold">[BREAKING]</span> {tickerHeadlines}
          </div>
        </div>
      </div>
      ...
      <section className="w-full relative py-8 sm:py-12 px-4 sm:px-12 border-b border-cyan-900/40 bg-[#040708] overflow-hidden">
        ...
        <div className="relative z-10 max-w-[1600px] mx-auto space-y-8">
          ...
          <div className="text-center space-y-3">
            <div className="inline-flex items-center gap-2.5 px-4 py-1.5 bg-cyan-950/90 border border-cyan-500/80 text-cyan-300 font-bold text-xs tracking-[0.25em] uppercase chamfer-corner shadow-hud-cyan">
              <Newspaper className="w-4 h-4 text-cyan-400 animate-pulse" />
              <span>MOLTOLOGY NEWS NETWORK // CENTRALIZED DISPATCH HUB</span>
            </div>
            ...
            <h1 className="font-grotesk font-black text-3xl sm:text-5xl lg:text-6xl text-gray-100 uppercase tracking-tight leading-none drop-shadow-2xl">
              OFFICIAL NEWS & <span className="text-cyan-400">AI TELEMETRY</span>
            </h1>

            <p className="text-xs sm:text-sm text-gray-300 max-w-2xl mx-auto font-sans leading-relaxed">
              Your centralized platform for live dispatches, agentic AI developments, test-time compute benchmarks, autonomous swarms, and official organization updates.
            </p>
          </div>
          ...
          <div className="pt-4 max-w-4xl mx-auto flex flex-col sm:flex-row items-center gap-4">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search news dispatches, field reports, or AI concepts..."
                value={searchQuery}
        ...
        {filteredPosts.length === 0 ? (
          <div className="text-center py-20 chitin-card border border-cyan-900/40 p-12 chamfer-corner">
            ...
            <p className="text-xs text-gray-400 mt-2 font-mono">
              Adjust your search query or select another news desk category.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredPosts.map((post, idx) => (
              <ScrollReveal key={post.slug} animation="fade-up" delayMs={idx * 100}>
                <article
                  onClick={() => navigate({ to: `/news/$slug`, params: { slug: post.slug } })}
