 ...
 */
export const getUserStatsHandler = async ({ data, context }: ServerFnArgs<{ token?: string }>) => {
  const auth = await resolveWriteAuth({ data, context, requireAuth: false })
  if (!auth) return null
 ...
 */
export const updateUserStatsHandler = async ({ data, context }: ServerFnArgs<UserStatsInput>) => {
  const auth = await resolveWriteAuth({ data, context })
 ...
 */
export const createAIThreadHandler = async ({ data, context }: ServerFnArgs<CreateAIThreadInput>) => {
  const auth = await resolveWriteAuth({ data, context })
