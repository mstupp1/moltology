...
export interface FullscreenDocumentItem {
  id: string
  title: string
  subtitle?: string
}

interface FullscreenDocumentReaderProps {
  items: FullscreenDocumentItem[]
  activeIndex: number
  overlayLabel: string
  tocToggleTitle?: string
  mobileHint?: string
  renderItem: (context: { zoom: number; pageWidth: number }) => React.ReactNode
  onPrev: () => void
  onNext: () => void
...
/**
 * Immersive fullscreen document reader shared by the codex and journal. Owns
...
export function FullscreenDocumentReader({
  ...
  activeIndex,
  overlayLabel,
  tocToggleTitle = 'Toggle Table of Contents Index',
  mobileHint = 'Tap the page to rest the chrome',
  renderItem,
  onPrev,
  onNext,
  onSelectItem,
    ...
    >
        ...
        <div className="codex-reader-chrome-bar flex items-center justify-between gap-2 px-2.5 sm:px-4 py-2 bg-[#0e1415]/92 backdrop-blur-md border-b border-[#293635] pt-[max(0.5rem,env(safe-area-inset-top))]">
          <div className="flex items-center gap-2 min-w-0">
            <button
              ...
              title={tocToggleTitle}
              aria-label={tocToggleTitle}
            >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              aria-label="Exit Fullscreen Overlay (ESC)"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
            ...
            <div className="absolute md:relative inset-y-0 left-0 w-[min(20rem,86vw)] bg-[#0e1415] border-r border-[#293635] p-3 overflow-y-auto space-y-1.5 z-20 font-sans pt-[4.5rem] md:pt-3">
              {items.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className="text-[10px] font-sans text-[#00ffff]">{item.id}</div>
      ...
      >
        <div className="flex items-center justify-between gap-2 px-3 py-2 bg-[#0e1415]/92 backdrop-blur-md border-t border-[#293635] pb-[max(0.5rem,env(safe-area-inset-bottom))]">
          ...
          <span className="text-[10px] font-sans text-[#839493]">{mobileHint}</span>
          <button
            onClick={onNext}
            disabled={activeIndex >= items.length - 1}
            className="px-3 py-2 text-[10px] font-bold uppercase tracking-wider border border-[#293635] text-[#dfe3e3] disabled:opacity-30 chamfer-corner"
