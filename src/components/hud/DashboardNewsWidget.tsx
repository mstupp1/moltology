...
export function DashboardNewsWidget({ isLoading = false }: DashboardNewsWidgetProps) {
  ...
  return (
    <HudGhostWidget isLoading={isLoading} skeleton={<DashboardNewsGhost />}>
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="font-grotesk text-sm font-bold text-[#dfe3e3] tracking-wider uppercase flex items-center gap-2">
              ...
              MOLTNATION INTELLIGENCE & NEWS FEED
            </h2>
            <div className="flex items-center gap-1.5 px-2 py-0.5 bg-[#ff5540]/15 border border-[#ff5540]/40 text-[#ff5540] text-[10px] font-mono font-bold chamfer-corner">
              <Radio className="w-3 h-3 text-[#ff5540] animate-pulse" />
              <span>LIVE FEED</span>
            </div>
          </div>
          <p className="text-xs text-[#839493] mt-0.5">
            Real-time patriot intelligence, swarm architecture updates, and benthic news dispatches.
          </p>
        </div>

        <button
          onClick={() => handleOpenNewsPage()}
          className="text-xs font-bold text-[#00ffff] hover:text-white flex items-center gap-1 transition-colors self-start sm:self-auto shrink-0 bg-[#070b0b] border border-[#3a4a49] px-2.5 py-1 chamfer-corner hover:border-[#00ffff]/60"
        >
      ...
      <div className="bg-[#04070a] border border-[#3a4a49] py-1.5 px-3 flex items-center gap-2 text-xs overflow-hidden chamfer-corner">
        <div className="flex items-center gap-1 px-2 py-0.5 bg-red-950/90 border border-red-500/80 text-red-400 font-extrabold uppercase tracking-wider text-[9px] shrink-0 chamfer-corner">
          <TrendingUp className="w-3 h-3 text-red-500" />
          <span>BREAKING</span>
        </div>
        <div className="overflow-hidden whitespace-nowrap text-[#00ffff]/90 text-[11px] font-mono flex-1">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
        <span className="text-[10px] text-[#839493] font-bold uppercase tracking-wider shrink-0 flex items-center gap-1 mr-1">
        ...
        {categories.map((cat) => (
          <button
            ...
            className={`px-2.5 py-1 text-[10px] font-bold font-mono uppercase tracking-wider chamfer-corner transition-all shrink-0 ${
              selectedCategory === cat
                ? 'bg-[#00ffff] text-black font-extrabold shadow-[0_0_10px_rgba(0,255,255,0.4)]'
      ...
      {/* Content Layout: Featured Post Card + Secondary Posts List */}
      {featuredPost && (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 pt-1">
          {/* Spotlight Hero Card (7 cols on md) */}
          <div
          ...
          {/* Secondary Posts List (5 cols on md) */}
