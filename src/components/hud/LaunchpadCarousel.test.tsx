...
describe('LaunchpadCarousel Component', () => {
  ...
  it('caps MoltNation News panel height on mobile to match desktop bento height', () => {
    render(<LaunchpadCarousel />)

    const panel = screen.getByTestId('moltnation-news-panel')
    const column = panel.parentElement
    expect(column).not.toBeNull()
    expect(column!.className).toMatch(/h-\[785px]/)
    expect(column!.className).toMatch(/max-h-\[785px]/)
  })
